...
#include "daisysp.h"
    ...
    // Sized for the pool's only current user: two daisysp::ReverbSc
    // instances (one per deck's Flux reverb mode). Adjust if more
    // SDRAM-resident allocations are added.
    #define INFS_SDRAM_POOL_SIZE (2 * (sizeof(daisysp::ReverbSc) + alignof(daisysp::ReverbSc)))
#endif
